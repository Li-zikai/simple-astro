...
echo "  生产升级示例（先确认目标集群中已有 gamemac release/Deployment/Pod）:"
echo "    helm upgrade gamemac ${CHART_OCI_REPO}/${CHART_NAME} \\"
echo "      --version ${VERSION} -n gamehub --plain-http --reuse-values \\"
echo "      --set image.tag=${VERSION} --wait --timeout 5m"
