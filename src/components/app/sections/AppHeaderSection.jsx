import { BrandTextIcon } from "@/components/app/icons"
import { useI18n } from "@/i18n/context"

const languageOptions = [
  { locale: "zh", shortLabel: "CN", label: "简体中文", href: "/zh-cn/" },
  { locale: "en", shortLabel: "EN", label: "English", href: "/en/" },
  { locale: "ru", shortLabel: "RU", label: "Русский", href: "/ru/" },
]

export function AppHeaderSection() {
  const { t, locale } = useI18n()
  const currentLanguage = languageOptions.find((option) => option.locale === locale) || languageOptions[0]

  return (
    <header className="app-header">
      <div className="header-inner">
        <div className="header-brand">
          <img src="/gamehub-mark.svg?v=4" alt={t.brandAlt} className="header-brand-icon" />
          <BrandTextIcon locale={locale} />
        </div>
        <div className="header-action">
          <a
            className="header-github-link"
            href="https://github.com/gamesir-labs/gamehub-for-mac"
            target="_blank"
            rel="noopener noreferrer"
            aria-label="GitHub"
          >
            <svg viewBox="0 0 16 16" width="20" height="20" fill="currentColor">
              <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z" />
            </svg>
          </a>
          <details className="header-language-menu">
            <summary className="header-lang-switch" aria-label={t.languageMenu.label}>
              <span>{currentLanguage.shortLabel}</span>
              <svg className="header-lang-chevron" viewBox="0 0 12 12" aria-hidden="true">
                <path d="m3 4.5 3 3 3-3" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </summary>
            <div className="header-language-options" aria-label={t.languageMenu.label}>
              {languageOptions.map((option) => (
                <a
                  key={option.locale}
                  className="header-language-option"
                  href={option.href}
                  aria-current={option.locale === locale ? "page" : undefined}
                >
                  <span>{option.label}</span>
                  <span className="header-language-option-code">{option.shortLabel}</span>
                </a>
              ))}
            </div>
          </details>
        </div>
      </div>
    </header>
  )
}
